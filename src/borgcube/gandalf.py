...
def bold(text, **kwargs):
    return colored(text, attrs=['bold'], **kwargs)


def conf():
    try:
        return os.path.abspath(os.environ['BORGCUBE_CONF'])
    except KeyError:
        pass

    base = os.environ.get('XDG_CONFIG_HOME',
                          os.path.join(os.path.expanduser('~' + os.environ.get('USER', '')), '.config'))
    return os.path.abspath(os.path.join(base, 'borgcube', 'conf.py'))
...
def gandalf():
    ...
    msg = 'Should I generate a systemd service file for you? [Y/n]'
    if yes(msg, default=True, retry_msg=msg):
        import grp
        unit = textwrap.dedent("""
        [Unit]
        Description=Backup system built on Borg Backup
        ...
        # The daemon doesn't require any physical devices, so we remove them from
        # it's cgroup.
        PrivateDevices=yes

        # Only signal the main process, not the entire cgroup. This way
        # this allows borgcubed to shutdown with much grace, instead of a random
        # process killing spree. (Within timely limits, TimeoutStopSec, 90s by default).
        KillMode=process

        # In case the main daemon crashes systemd will automatically restart it
        # with this directive.
        Restart=on-failure
        ...
        """).strip().format(
            ...
            borgcubed=shutil.which('borgcubed'),
            env=quote('BORGCUBE_CONF=' + cf)
        )
        print(bold('--- borgcube.service ---'))
        print('# Generated by borgcube-gandalf on', datetime.now().isoformat(' '))
        print('# User', getpass.getuser() + '@' + socket.getfqdn())
        print(unit)
        print(bold('--- snip ---'))
        print()
        print('Place this file (with su privileges) in /etc/systemd/system and run')
        print(' # systemctl enable borgcube')
        print(' # systemctl start borgcube')
        print('to have it start automatically.')
